...
export const MobileHeader = ({ navigation }) => {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {active && <LinksOverlay nav={navigation} />}
      </AnimatePresence>
    </>
  );
};
...
const LinksContainer = ({ nav }) => {
  return (
    <motion.div className='space-y-6 p-12 pl-4 md:pl-20'>
      {nav.map((l, idx) => {
        return (
          <NavLink key={l.name} href={l.link} idx={idx}>
            {l.name}
          </NavLink>
        );
      })}
  ...
  return (
    <>
      <motion.div
        ...
        className='fixed z-10 rounded-xl bg-gradient-to-br from-green-700 to-green-800 shadow-lg shadow-green-800/20'
      />
